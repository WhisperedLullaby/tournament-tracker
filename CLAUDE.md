...
| `pickup_registrations` | Per-player sign-ups by position. `user_id` nullable (guest rows); self-sign-up requires auth, but organizers can manually add a player without an account via `POST /api/pickup/[sessionId]/registrations` (guest row, `user_id` null). Cancel requires auth (own registration only); organizers can remove any registration via `DELETE /api/pickup/[sessionId]/registrations/[regId]`. Insert paths share `addPickupRegistration()` (session-row lock + capacity/waitlist numbering, one transaction); removal paths share `removePickupRegistration()` (delete + waitlist promote + renumber, one transaction). Both are blocked once the session is active/completed. The public GET list endpoint strips `email`/`userId` for non-organizers (`PublicPickupRegistration`) |
...
There was no UI to cancel a registration (the self-cancel API existed but nothing called it) and no way for an organizer to remove a player who said they can't come. Added: (1) `DELETE /api/pickup/[sessionId]/registrations/[regId]` — organizer-only, removes any registration including legacy guest rows; (2) the shared `removePickupRegistration()` helper in `pickup-queries.ts` (delete + waitlist promote + renumber, one transaction) now backs both this route and the self-cancel route — promotion fires for both `registered` and `attended` (checked-in) players; (3) roster rows on the session page show a remove button with inline confirm — organizers see it on every player, a signed-in player sees it on their own row; (4) a "Can't make it? Withdraw" button (inline confirm) under the "You're registered" badge; (5) `refreshRegistration()` exposed from `pickup-context.tsx` so the page resyncs after withdrawal. Removal works while the session is `upcoming` or `attendance` and is blocked once active/completed, mirrored client- and server-side.

### ✅ Pickup session status can be changed from Settings — RESOLVED
Status transitions were one-way and implicit (taking attendance flips `upcoming → attendance`; generating a series flips `attendance → active`) with no way back — an organizer who started check-in early couldn't reopen the roster. The Settings page now has a "Session Status" card (select + explicit Update button, separate from the settings form) that can move a session to any of the four statuses via the existing `PATCH /api/pickup/[sessionId]` (which now validates the status value server-side). Moving backward keeps existing check-in marks and generated lineups; re-taking attendance overwrites marks, and non-completed series can be deleted from the Lineups page.
